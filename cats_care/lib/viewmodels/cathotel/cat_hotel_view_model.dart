...
class CatHotelViewModel extends ChangeNotifier {
  ...
  Future<void> loadCatHotelData() async {
    try {
      final String jsonString = await rootBundle.loadString(
          'assets/cathotels.json');
      final List<dynamic> jsonList = json.decode(jsonString);
      _originalCatHotelData =
          jsonList.map((json) => CatHotel.fromJson(json)).toList();
      _filteredCatHotelData = List.from(_originalCatHotelData);
      notifyListeners();
    } catch (e) {
      print('Error loading cathotel data: $e');
    }
  }
  ...
  Future<void> saveBooking({
    ...
    try {
      ...
      print('Error saving bookinghotel: $e');
    }
  }
